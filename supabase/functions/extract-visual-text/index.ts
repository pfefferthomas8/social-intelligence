...
Deno.serve(async (req: Request) => {
  ...
  await fetch(`${SUPABASE_URL}/rest/v1/instagram_posts?id=eq.${post_id}`, {
    method: 'PATCH', headers: dbHeaders(), body: JSON.stringify({ visual_text_status: 'pending' })
  })

  const imageData = await fetchImageAsBase64(thumbnail_url)
  if (!imageData) {
    await fetch(`${SUPABASE_URL}/rest/v1/instagram_posts?id=eq.${post_id}`, {
      method: 'PATCH', headers: dbHeaders(), body: JSON.stringify({ visual_text_status: 'error' })
    })
    ...
    body: JSON.stringify({
      ...
      messages: [{
        ...
        content: [
          ...
          { type: 'text', text: `Extrahiere ALLE sichtbaren Texte aus diesem Instagram-Video-Frame.
...
Gib nur den extrahierten Text aus, keine Erklärungen.` }
  ...
  await fetch(`${SUPABASE_URL}/rest/v1/instagram_posts?id=eq.${post_id}`, {
    method: 'PATCH', headers: dbHeaders(), body: JSON.stringify({ visual_text: visualText, visual_text_status: 'done' })
  })
